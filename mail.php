...
$site_top = "https://sbkunren.xsrv.jp/202306ec/25/index.html";

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
$to = "2023yabuuchi@gmail.com";
...
$subject = "Ayano's portfoliosへのメッセージ";
...
$refrom_name = "Ayano's portfolios";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "メッセージありがとうございました！";
...
if (($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1) {
	...
	if ($useToken == 1 && $confirmDsp == 1) {
		...
		if (isset($_POST['mailform_token']))
			unset($_POST['mailform_token']); //トークン破棄
	}
			...
			<?php } else { ?>
					...
					<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
						...
						<p align="center"><input type="hidden" name="mail_set" value="confirm_submit">
							...
							<input type="button" value="　編集する　" onClick="history.back()">
						</p>
					</form>
			<?php } ?>
			</div><!-- /formWrap -->
...
if (($jumpPage == 0 && $sendmail == 1) || ($jumpPage == 0 && ($confirmDsp == 0 && $sendmail == 0))) {
	...
	<?php } else { ?>
		<div class="container">
			<p>メッセージを頂き、ありがとうございました！</p>
			<p>送信完了メールを送らせていただきました。<i class="fa-solid fa-otter"></i></p>
			<br /><br />
